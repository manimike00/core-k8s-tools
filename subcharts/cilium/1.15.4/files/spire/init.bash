...
CILIUM_AGENT_SELECTORS="-selector k8s:ns:{{ .Values.Namespace }} -selector k8s:sa:{{ .Values.serviceAccounts.cilium.name }}"
CILIUM_OPERATOR_SPIFFE_ID="spiffe://{{ .Values.authentication.mutual.spire.trustDomain }}/cilium-operator"
CILIUM_OPERATOR_SELECTORS="-selector k8s:ns:{{ .Values.Namespace }} -selector k8s:sa:{{ .Values.serviceAccounts.operator.name }}"
